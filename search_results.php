...
<h4>Purchase product</h4>
        <form action="includes/insert.inc.php" method="post">
            ...
            <br>
                <h4>Select broker</h4>
            <select name="select_consumer" Select>
                <?php

                $sql_broker2 = "SELECT broker_id FROM broker";
                $records_broker2 = mysqli_query($conn, $sql_broker2);

                while ($row = mysqli_fetch_array($records_broker2)) {
